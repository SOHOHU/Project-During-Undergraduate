...
def main(m=0.9):
    ...
    df = load_data("E:\论文2数据集\G5\cnae+9\cnae.xlsx")
    ...
    print(f"初始数据集：多数类样本数量 = {majority_count}，少数类样本数量 = {minority_count}")

    # 计算初始忠诚度
    loyalties = calculate_loyalty(X, y, k=5)
    # 找出边界样本的索引
    boundary_indices = np.where((loyalties >= 0) & (loyalties <= m))[0]

    # 统计多数类和少数类边界样本的数量
    majority_boundary_count = np.sum(y[boundary_indices] == majority_class)
    minority_boundary_count = np.sum(y[boundary_indices] == minority_class)

    print(f"多数类边界样本数量为: {majority_boundary_count}")
    print(f"少数类边界样本数量为: {minority_boundary_count}")

    # 提取边界样本的特征和标签
    X_boundary = X[boundary_indices]
    y_boundary = y[boundary_indices]

    # 根据多数类样本数量确定目标少数类样本数量
    target_minority_count = majority_count

    # 确保目标少数类样本数量不小于原始少数类样本数量
    if target_minority_count < minority_count:
        target_minority_count = minority_count

    # 检查边界样本中的类别数量
    unique_classes = np.unique(y_boundary)
    ...
    # 输出过采样后数据集多数类和少数类样本数量
    class_counts = np.bincount(y)
    majority_count = class_counts[majority_class]
    minority_count = class_counts[minority_class]
    print(f"过采样后：多数类样本数量 = {majority_count}，少数类样本数量 = {minority_count}")
    ...
    # 输出删除样本后数据集多数类和少数类样本数量
    class_counts = np.bincount(y)
    majority_count = class_counts[majority_class]
    minority_count = class_counts[minority_class]
    print(f"删除样本后：多数类样本数量 = {majority_count}，少数类样本数量 = {minority_count}")

    df_result = pd.DataFrame(np.hstack([X, y.reshape(-1, 1)]), columns=list(df.columns))
    save_data(df_result, "E:\论文2数据集\G5\cnae+9\loyaltysmote.xlsx")
